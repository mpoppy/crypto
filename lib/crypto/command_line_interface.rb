...
class CLI
  ...
  def list
    ...
    Currency.create_from_set(crypto_array)
  ...
  def menu
    puts "Enter the number (1-30) for the cryptocurrency for which you would like to get more information on. Type 'exit' to leave the program. "
    input = gets.strip.downcase
    if input != "exit"
      input = input.to_i - 1
